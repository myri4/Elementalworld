...
namespace wc {
	...
	public:
		static void Init()
		{
			...
			logSinks[1] = std::make_shared<spdlog::sinks::basic_file_sink_mt>("log/Server.log", true);

			logSinks[0]->set_pattern("%^[%D %T][%l] %n: %v%$");
			logSinks[1]->set_pattern("%^[%D %T][%l] %n: %v%$");

			GetLogger() = std::make_shared<spdlog::logger>("Server", begin(logSinks), end(logSinks));
