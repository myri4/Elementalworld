...
namespace wc {
	struct RenderPass : public RendererObject<VkRenderPass> {
		...
		VkResult Create(const VkRenderPassCreateInfo& render_pass_info) { return vkCreateRenderPass(VulkanContext::GetDevice(), &render_pass_info, VulkanContext::GetAllocator(), &m_RendererID); }

		void Destroy() { 
			vkDestroyRenderPass(VulkanContext::GetDevice(), m_RendererID, VulkanContext::GetAllocator());
	...
	struct Framebuffer : public RendererObject<VkFramebuffer> {
		...
		void Create(const VkFramebufferCreateInfo& framebuffer_create_info) { vkCreateFramebuffer(VulkanContext::GetDevice(), &framebuffer_create_info, VulkanContext::GetAllocator(), &m_RendererID); }
