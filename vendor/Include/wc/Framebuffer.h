	...
	struct FramebufferWC {
		...
		*/
		uint32_t addAttachment(AttachmentCreateInfo createinfo)
		{
			...
			attachment.view.Create(imageView);
			...
			else if (createinfo.presentable) attachment.description.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
		...
		*/
		VkResult Create(const glm::ivec2& size)
		{
			...
			renderPass.Create(renderPassInfo);
			...
			framebuffer.Create(framebufferInfo);
