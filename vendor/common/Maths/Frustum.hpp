...
namespace wc {
    ...
    struct AABB {
        ...
        AABB() {}
        AABB(const glm::vec3& pos, const glm::vec3& dims) : position(pos), dimensions(dims) {}

        glm::vec3 getVN(const glm::vec3& normal) const
        {
            glm::vec3 res = position;

            if (normal.x < 0) res.x += dimensions.x;
            if (normal.y < 0) res.y += dimensions.y;
            if (normal.z < 0) res.z += dimensions.z;
