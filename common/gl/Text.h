...
namespace gl {
    enum class TextStatus {
        OK, COULD_NOT_INIT_FREETYPE_LIB, FAILED_TO_FIND_FONT, FAILED_TO_LOAD_FONT, FAILED_TO_LOAD_GLYPH
    };
    class Text {
    public:
        Text() {
        ...
        TextStatus Create(const char* fontFileLoc, const char* vs, const char* fs, glm::mat4 projection,int glyphs = 128) {
            ...
            {
                return TextStatus::COULD_NOT_INIT_FREETYPE_LIB;
            }

            // find path to font
            if (fontFileLoc == "")
            {
                return TextStatus::FAILED_TO_FIND_FONT;
            }

            // load font as face
            FT_Face face;
            if (FT_New_Face(ft, fontFileLoc, 0, &face)) {
                return TextStatus::FAILED_TO_LOAD_FONT;
            }
            else {
