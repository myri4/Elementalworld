...
namespace wc {
	...
	namespace Renderer3D {
		...
		void CreateLinePipeline(const wc::RenderPass& renderPass, const VkDescriptorBufferInfo& ubo) {
			...
			wc::descriptorAllocator.allocate(lineSet, lineShader.getDescriptorLayout());

			wc::DescriptorWriter writer;

			writer.dstSet = lineSet;
		...
		void Flush(const bool render = true) {
			...
			if (render) {
				...
				cmd.BindDescriptorSet(VK_PIPELINE_BIND_POINT_GRAPHICS, 0, lineShader.getPipelineLayout(), lineSet);
				lineShader.Bind(cmd);
		...
		void Bind(const wc::CommandBuffer& cmd) {
			cmd.BindDescriptorSet(VK_PIPELINE_BIND_POINT_GRAPHICS, 0, shader.getPipelineLayout(), descriptorSet);
			shader.Bind(cmd);
