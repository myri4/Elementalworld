	...
	public:
		...
		void OnInput(float deltaTime) {
			...
			if (!console && !inventory)
			{
				...
				else if (Key::getKey(Key::LeftShift) && p.flying)
					p.acceleration.y -= p.MovementSpeed;

				if (Key::getKey(Key::C)) { addFOV = -80.f; m_MouseSensitivity = Settings::ZoomMouseSensitivity; }
				else
				...
				float ms = 1.f / m_MouseSensitivity;
			...
			if (Key::getKey(Key::F1)) renderGUI = !renderGUI;
			
			if (Key::getKey(Key::F8)) debug_menu = !debug_menu;
			if (Key::getKey(Key::Enter) && console) {
				if (consoleBuffer[0] == '/') { 
			...
			bool bBreak = Mouse::getMouse(Mouse::LEFT);
			bool bPlace = Mouse::getMouse(Mouse::RIGHT);

			if (Key::getKey(Key::D1)) blockHolding = campfire;
			if (Key::getKey(Key::D2)) blockHolding = murshroom;
