	...
	private:
		...
		void OnInput() {
			...
			if (window.hasFocus()) {
				...
				if (menuMode == MenuMode::ESCMENU && Key::getKey(Key::Escape))
					ChangeMenu(MenuMode::GAME);


				if (menuMode == MenuMode::GAME && Key::getKey(Key::Escape)) ChangeMenu(MenuMode::ESCMENU);

				if (Key::getKey(Key::F3)) Resize();
			}
		}
		//----------------------------------------------------------------------------------------------------------------------
		void OnCreate() {
		...
		void OnUpdate() {
			...
			if (menuMode == MenuMode::WORLD_CREATION) {
				...
				if (ImGui::Button("Create") || Key::getKey(Key::Enter)) {
